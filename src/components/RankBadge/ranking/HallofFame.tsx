...
export function HallOfFame() {
  const [data, setData] = useState<HallOfFameUser[]>([])
  ...
  const fetchRankings = async () => {
    try {
      ...
      console.error("명예의 전당 데이터 불러오기 실패:", error)
    } finally {
      setLoading(false)
    }
  }
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center py-12">
        <div className="text-gray-600">명예의 전당을 불러오는 중...</div>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      {data.length === 0 ? (
        <Card className="p-8 text-center">
          <p className="text-gray-600">명예의 전당 데이터가 없습니다.</p>
        </Card>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {data.map((legend, index) => (
            <motion.div key={`${legend.userId}-${legend.rank}`} initial={{ opacity: 0, scale: 0.8, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ delay: index * 0.2 }}>
                ...
                <div className="relative z-10 flex flex-col items-center text-center">
                  ...
                  <div className="w-full space-y-3 mb-6">
                    <div className="bg-white/60 backdrop-blur rounded-lg p-3">
                      ...
                      <p className="text-xl text-amber-900">{(legend.xp ?? 0).toLocaleString()} XP</p>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-white/60 backdrop-blur rounded-lg p-3">
                        <p className="text-xs text-amber-700 mb-1">레벨</p>
                        <p className="text-lg text-amber-900">{calculateLevel(legend.xp ?? 0)}</p>
                      </div>
                      <div className="bg-white/60 backdrop-blur rounded-lg p-3">
                        <p className="text-xs text-amber-700 mb-1">연속일</p>
                        <p className="text-lg text-amber-900">{legend.streak ?? 0}일</p>
                      </div>
                    </div>
                  </div>
